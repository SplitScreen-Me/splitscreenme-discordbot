...
exports.execute = async (DiscordBot, receivedMessage, args) => {
    ...
    if (cmd.config.aliases.length) embed.addFields({ name: 'Aliases' , value: `\`\`\`${cmd.config.aliases.join(` , `)}\`\`\`` } )
    if (cmd.config.usage) embed.addFields({ name: 'Usage  [mandatory] \<optional\>' , value: `\`\`\`${cmd.config.usage}\`\`\`` } )
    //if (cmd.config.aliases) embed.addFields({ name: 'Parameters' , value: `\`\`\`${cmd.config.parameters}\`\`\`` } )
    if (cmd.config.example) embed.addFields({ name: 'Example' , value: `\`\`\`${cmd.config.example}\`\`\`` } )
